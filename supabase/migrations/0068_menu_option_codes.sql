-- 0068_menu_option_codes.sql
-- Give a wing flavour its menu code back.
--
-- The printed menu numbers the ten flavours NY1 to NY10, and the storefront
-- could not show that, because the number lived in two places this schema
-- cannot read. On the photographs it is burned into the pixels as an orange
-- corner triangle, and scripts/lib/image-pipeline.ts crops that off during
-- ingest, correctly: a code baked into a photograph cannot be reordered,
-- corrected or translated, and it lands wherever the crop happens to put it.
-- Everywhere else it was simply never captured, because menu_options had no
-- column to hold it while menu_items has had `code` since 0003.
--
-- So the burgers and the hotdogs carried BB1 to BB5 and H1 to H5 and every
-- flavour showed nothing, which is the gap the owner spotted against
-- nybuffalobrads.com.ph/our-menu-test.
--
-- WHERE THE TEN NUMBERS COME FROM.
-- ---------------------------------------------------------------------------
-- Read out of the badges on the 2025/03 shoot in C:/dev/nybb-assets, which
-- carries all ten flavours and numbers them NY1 to NY10 in exactly the order
-- this catalog already lists them. They agree with the live website.
--
-- The 2024/05 shoot is NOT the source and must not be used as one: it numbers
-- the same photographs differently, with Brad's Gravy at NY3 and Sweet Spicy
-- at NY10, from a nine-flavour menu that predates Brad's Gravy being the
-- tenth. Seven of the ten agree between the two shoots, which is exactly the
-- kind of near-miss that reads as confirmation until it is checked.

alter table menu_options add column code text;

comment on column menu_options.code is
  'The menu''s own code for this option: NY1, NY7. Wing flavours only; null '
  'everywhere else, the same way menu_items.code is null for an item the '
  'printed menu does not number.';

-- Keyed on slug, never on sort_order: the flavour grid is owner-reorderable
-- from Phase 4, and a reorder must not renumber the printed menu.
...
join menu_option_groups g on g.slug = 'wing-flavour'
where mo.group_id = g.id
  and mo.slug = v.slug;

-- Recreated from 0057 verbatim but for the one added key, which is the whole
-- reason this file restates 185 lines it does not otherwise change.
