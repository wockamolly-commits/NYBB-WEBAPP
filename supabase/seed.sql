...
insert into menu_categories (slug, name, blurb, sort_order) values
  ('chicken-wings', 'Chicken', 'Ten flavours, five levels of heat. The reason the place exists.', 0),
  ('ny-specials', 'NY Specials', 'Ribs off the bone, and nuggets by the box.', 1),
  ('hunger-busters', 'Hunger Busters', 'A burger, fries and a drink.', 2),
  ('breaktime-treats', 'Breaktime Treats', 'A dog, fries and a drink.', 3),
  ('value-meals', 'Value Meal', 'Rice, a main, and change from a note.', 4),
  ('ny-burgers', 'NY Burgers', 'Eight burgers, beef and chicken, on their own.', 5),
  ('pasta', 'Pasta', 'Two plates, solo or as a meal.', 6),
  ('ny-hotdogs', 'Hotdog And Sausage', 'On their own, numbered H1 to H5.', 7),
  ('sides', 'Fries & Sides', 'The supporting cast.', 8),
  ('rice-meals', 'Rice Meals', 'A sausage or a fillet, and rice.', 9),
  ('iced-coffee', 'Coffee Series', 'Cold, and cheaper than the mall.', 10),
  ('beverages', 'Beverages', 'Cold drinks by the cup and by the bottle.', 11),
  ('waffles', 'Waffles', 'On their own, or with an iced coffee.', 12)
on conflict (slug) do update set
  name = excluded.name,
...
insert into menu_items (
  ...
  ((select id from menu_categories where slug = 'ny-specials'), 'ribs-original', 'Original Ribs', null, null, '2025/03/RIBS-ORIG.jpg', 'cutout', null, true, true, 0),
  ((select id from menu_categories where slug = 'ny-specials'), 'ribs-spicy', 'Spicy Ribs', null, null, '2025/03/RIBS-SPICY.jpg', 'cutout', null, false, true, 1),
  ...
  ((select id from menu_categories where slug = 'breaktime-treats'), 'jalapeno-cheesedog-meal', 'Jalapeno Cheesedog Meal', null, null, '2024/05/Jalapeno-Cheese-Dog.jpg', 'cutout', 'Foodpanda delivery list price, SM City Cebu, captured 2026-09-07. Includes the delivery channel markup and is not a pickup price. Inactive until the owner confirms. See docs/MENU-PRICES-TO-CONFIRM.md.', false, false, 1),
  ...
  ((select id from menu_categories where slug = 'ny-hotdogs'), 'jalapeno-cheese-dog', 'Jalapeno Cheesedog', 'H2', null, '2024/05/Jalapeno-Cheese-Dog.jpg', 'cutout', null, false, true, 1),
  ...
  ((select id from menu_categories where slug = 'sides'), 'french-fries', 'NY Fries', null, null, null, null, 'Our list has one French Fries price of 128. Foodpanda sells NY Fries in three sizes (103 / 172 / 195), so the single price cannot be matched to a size. Renamed to NY Fries to match the current menu; the sizes need the owner.', false, true, 0),
  ...
  ((select id from menu_categories where slug = 'rice-meals'), 'hungarian-rice-meal', 'Hungarian With Rice', null, null, null, null, 'Our list has one price of 189. Foodpanda splits this into Solo 213 and Meal 248, so which of the two our 189 is remains unknown.', false, true, 0),
  ...
  ((select id from menu_categories where slug = 'beverages'), 'cucumber-lemonade', 'Cucumber Lemonade', null, null, null, null, 'Foodpanda delivery list price, SM City Cebu, captured 2026-09-07. Includes the delivery channel markup and is not a pickup price. Inactive until the owner confirms. See docs/MENU-PRICES-TO-CONFIRM.md.', false, false, 0),
  ((select id from menu_categories where slug = 'beverages'), 'lemon-iced-tea', 'Lemon Iced Tea', null, null, null, null, 'Foodpanda delivery list price, SM City Cebu, captured 2026-09-07. Includes the delivery channel markup and is not a pickup price. Inactive until the owner confirms. See docs/MENU-PRICES-TO-CONFIRM.md.', false, false, 1),
...
insert into item_variations (
  ...
  ((select id from menu_items where slug = 'chicken-wings'), 'boneless-half', 'Boneless half, 6 pieces', 'BL HALF', 32900, false, 2),
  ((select id from menu_items where slug = 'chicken-wings'), 'boneless-full', 'Boneless full, 10 pieces', 'BL FULL', 52900, false, 3),
  ...
  ((select id from menu_items where slug = 'rookie-burger-meal'), 'regular', 'Regular', 'REG', 20600, true, 0),
  ((select id from menu_items where slug = 'quarterback-burger-meal'), 'regular', 'Regular', 'REG', 32600, true, 0),
  ((select id from menu_items where slug = 'blt-burger-meal'), 'regular', 'Regular', 'REG', 42500, true, 0),
  ((select id from menu_items where slug = 'buffalo-chicken-burger-meal'), 'regular', 'Regular', 'REG', 40100, true, 0),
  ((select id from menu_items where slug = 'smokey-bbq-chicken-burger-meal'), 'regular', 'Regular', 'REG', 35000, true, 0),
  ((select id from menu_items where slug = 'honey-garlic-chicken-burger-meal'), 'regular', 'Regular', 'REG', 40100, true, 0),
  ((select id from menu_items where slug = 'cheezy-chicken-burger-meal'), 'regular', 'Regular', 'REG', 40100, true, 0),
  ((select id from menu_items where slug = 'brads-angus-burger-meal'), 'regular', 'Regular', 'REG', 34900, true, 0),
  ((select id from menu_items where slug = 'classic-hotdog-meal'), 'regular', 'Regular', 'REG', 20200, true, 0),
  ((select id from menu_items where slug = 'jalapeno-cheesedog-meal'), 'regular', 'Regular', 'REG', 24800, true, 0),
  ((select id from menu_items where slug = 'chili-cheesedog-meal'), 'regular', 'Regular', 'REG', 29400, true, 0),
  ((select id from menu_items where slug = 'hungarian-sandwich-meal'), 'regular', 'Regular', 'REG', 31700, true, 0),
  ...
  ((select id from menu_items where slug = 'chicken-wings-meal'), 'regular', 'Regular', 'REG', 22900, true, 0),
  ((select id from menu_items where slug = 'boneless-chicken-meal'), 'regular', 'Regular', 'REG', 22900, true, 0),
  ((select id from menu_items where slug = 'chicken-nuggets-meal'), 'regular', 'Regular', 'REG', 22900, true, 0),
  ((select id from menu_items where slug = 'burger-steak-meal'), 'regular', 'Regular', 'REG', 22900, true, 0),
  ...
  ((select id from menu_items where slug = 'brads-angus-burger'), 'regular', 'Regular', 'REG', 43600, true, 0),
  ((select id from menu_items where slug = 'smokey-bbq-chicken-burger'), 'regular', 'Regular', 'REG', 30900, true, 0),
  ...
  ((select id from menu_items where slug = 'french-fries-cheese'), 'regular', 'Regular', 'REG', 19000, true, 0),
  ((select id from menu_items where slug = 'french-fries-bbq'), 'regular', 'Regular', 'REG', 19000, true, 0),
  ((select id from menu_items where slug = 'french-fries-sour-cream'), 'regular', 'Regular', 'REG', 19000, true, 0),
  ((select id from menu_items where slug = 'mozzarella-sticks'), 'regular', 'Regular', 'REG', 29900, true, 0),
  ((select id from menu_items where slug = 'plain-rice'), 'regular', 'Regular', 'REG', 5700, true, 0),
  ...
  ((select id from menu_items where slug = 'iced-sweet-black'), 'regular', 'Regular', 'REG', 11900, true, 0),
  ...
  ((select id from menu_items where slug = 'cucumber-lemonade'), '8-oz', '8 oz', '8 OZ', 9100, true, 0),
  ((select id from menu_items where slug = 'cucumber-lemonade'), '16-oz', '16 oz', '16 OZ', 13700, false, 1),
  ((select id from menu_items where slug = 'cucumber-lemonade'), '22-oz', '22 oz', '22 OZ', 16000, false, 2),
  ((select id from menu_items where slug = 'lemon-iced-tea'), '8-oz', '8 oz', '8 OZ', 9100, true, 0),
  ((select id from menu_items where slug = 'lemon-iced-tea'), '16-oz', '16 oz', '16 OZ', 13700, false, 1),
  ((select id from menu_items where slug = 'lemon-iced-tea'), '22-oz', '22 oz', '22 OZ', 16000, false, 2),
  ((select id from menu_items where slug = 'bottled-water'), 'regular', 'Regular', 'REG', 5700, true, 0),
