...
> **Superseded 2026-09-07. The Hot Wings list above is stale; Foodpanda is the live menu.**
>
> The Our Menu page this was transcribed from has not been maintained. It still advertises the
> closed Sports Lounge, and its prices are years behind. The live source is the Foodpanda listing
> for SM City Cebu (vendor `ir88`), which is fetchable, unlike the website: see section 5.6 for how,
> including the trap that a vendor promo masks `price` and the real figure is `price_before_discount`.
>
> What the transcription above got wrong or missed:
>
> - **Ten wing flavours, not nine.** Brad's Gravy is the tenth. It was filed as Sports Lounge only
...
> - **Meals are their own products**, in their own categories (Hunger Busters for burgers,
>   Breaktime Treats for hotdogs), not a variation on the solo item.
> - **Missing entirely:** Iced Sweet Black, three flavoured fries, NY Fries in three sizes, Plain
>   Rice, Cucumber Lemonade and Lemon Iced Tea in three sizes each, Bottled Water, and three of the
>   four named Value Meals.
> - **Prices are 30 to 40 percent higher** on the current menu than transcribed above, and a few are
>   lower. Foodpanda's are delivery prices and carry that channel's markup, so they are not pickup
>   prices either. Neither source gives a correct pickup price.
>
> The catalog now mirrors the Foodpanda category structure. Twenty-three items are seeded
> `is_active = false` carrying the Foodpanda figure and a `pricing_note`, because
> `item_variations.price_cents` is `not null` and a guessed price on a live row is the one thing a
> server-authoritative model must not do. `docs/MENU-PRICES-TO-CONFIRM.md` lists every one; it is
> generated by `npm run build:prices-to-confirm`. Section 28 owns the question.
...
| D2 | **Single branch at launch, multi-branch-ready schema.** This is a proposal to the CEO. It must demo as one clean store while provably scaling to ten. | Every menu, pricing, hours, stock, order, and settings table carries `branch_id` from migration one. ~~The branch picker is fully built and hidden behind a flag.~~ **Superseded 2026-08-19: the counter picker ships at `/stores`, unhidden.** Hiding it did not make the product read as one clean store, it made the customer's most load-bearing question invisible: `get_pickup_slots(null)` resolved the first active branch by `sort_order`, so the shop somebody collected from was chosen for them and named only in a footnote at the bottom of checkout, while the cart screen promised "the branch you choose at checkout". The picker now lists all nine counters, marks the live one as choosable and the other eight as phone-only with their real numbers, and hides itself only where no order can be completed at all. Migration 0049 adds the `get_orderable_branches()` read this needs, because `branches` is RLS'd to staff. Never hardcode a branch. |
...
| Iced Coffee Series | 4 posters, text baked in | Corrected: the cup region carries no text, so all 4 are covered by a crop. See below. |
...
>
> **Narrowed again 2026-09-07. The Iced Coffee Series is covered.** "Text baked in" was read as
> "unusable" and it only ever meant "not usable whole". The four posters are one 1080x1080
> template: headline across the top, flavour name beside the cup, cup and garnish in the middle on
> the same flat orange the burger and hotdog cutouts already sit on. The region holding the cup
> carries no text, so a stated crop of `{ left: 110, top: 320, 690x690 }` yields four clean product
> tiles. This is the corner-badge argument applied a second time: unwanted pixels in a source are a
> crop, not a disqualification.
>
> `scripts/lib/image-pipeline.ts` grew an optional `crop` on `Source` for this. The badge is
> measured because it varies between sources; this does not vary, so it is stated once. The
> treatment window is taken inside the declared region, so a cropped source is framed by the same
> rules as an uncropped one.
>
> `2025/03/Ice-Coffee-FB-complete-no-price-1.jpg` is the same series as a four-cup lineup. It is a
> category image, not an item image, and is mapped to no row.
>
> That leaves **three** true gaps, all in Sides: French Fries, Hungarian Rice Meal and Chicken with
> Rice. The archive holds nothing that could cover them, so they are the whole of the photography
> ask, alongside the three soft wing flavours. They ship as `<NoPhotoTile>`.
>
...
**Phase 5, optional.** Loyalty points. Email. Recommendations. ~~The branch picker unhidden.~~ (Done 2026-08-19, see D2.) k6 load
...
2a. **What is the pickup price of each menu item?** Opened 2026-09-07. Our seeded prices come from
   the website's Our Menu page, which is stale; the only current source is Foodpanda, whose prices
   carry a delivery markup. Twenty-three items are switched off waiting on this. The full list, with
   the Foodpanda figure beside each blank, is `docs/MENU-PRICES-TO-CONFIRM.md`. Also open there:
   whether Value Meal Sets A/B/C still exist, which NY Fries size our single 128 is, whether the
   counter still sells Waffles, Hawaiian BBQ and Chicken with Rice, and whether the NY Umbrella and
   the Rainy Day Rush bundles belong in a permanent catalog.
