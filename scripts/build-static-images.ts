 ...
 * derivatives under public/img so the static pages have real photography, and
 * it shares the same transform rules so the eventual ingest is a change of
 * destination rather than a change of pipeline.
 *
...
const ARCHIVE = process.env.NYBB_ASSETS ?? "C:/dev/nybb-assets";
const ORIGINALS = path.join(ARCHIVE, "originals");
 ...
 */
type Treatment = "lifestyle" | "cutout" | "transparent" | "scene" | "mark";
...
async function build() {
  ...
  for (const key of keys) {
    ...
    const digest = createHash("sha256").update(buffer.data).digest("hex").slice(0, 8);
    const outputName = `${key}.${digest}.webp`;
    await writeFile(path.join(PUBLIC_DIR, outputName), buffer.data);
    ...
    console.log(`${key.padEnd(30)} ${String(info.width).padStart(4)}x${String(info.height).padEnd(4)} ${String(kb).padStart(4)} KB  <- ${source.file}`);
  }
